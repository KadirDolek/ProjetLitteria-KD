        ...
        <div className="w-3/4 bg-gray-700 h-152 rounded-4xl flex mx-auto flex-col gap-3 p-6">
          {currentItems.map((book) => (
            <div key={book.id} className="text-white px-2 gap-3 flex h-42">
             <img className="w-24 mt-3 transition-transform duration-300 ease-in-out hover:scale-125 cursor-pointer" src={book.image_url} alt="" /> <br />
             <div style={{fontSize:'12px'}}>
                <span className="text-2xl underline text-amber-50">{book.title}</span> <button className="cursor-pointer hover:underline hover:text-amber-700">➥ Voir les détails</button>
